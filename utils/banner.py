...
def print_banner():
    ...
    print(bline(f"  {YELLOW}⚡ NORMAL MODE {GRAY} │ {R}Fast direct OS control (default)      "))
    print(bline(f"  {CYAN}🤖 WORKING MODE{GRAY} │ {R}Full 11-agent pipeline for complex tasks"))
    print(' ' + GRAY + '╠' + '═' * (box_w - 2) + '╣' + R)
    print(bline(f"  {DGRAY}Quick Start:", DGRAY))
    print(bline(f"  {DGRAY}• Type any command naturally  (e.g. \"open chrome\")    ", DGRAY))
    print(bline(f"  {DGRAY}• \"mode working\" → switch to full AI pipeline          ", DGRAY))
    print(bline(f"  {DGRAY}• \"help\" → all commands   •   \"status\" → system health ", DGRAY))
    print(bline(f"  {DGRAY}• \"exit\" → quit                                        ", DGRAY))
    print(' ' + GRAY + '╚' + '═' * (box_w - 2) + '╝' + R)
    print()
