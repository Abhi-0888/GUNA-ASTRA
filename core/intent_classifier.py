...
_add(r"^(?:open|go to|browse to|visit|navigate to)\s+((?:https?://)?[\w.-]+\.\w{2,}(?:/\S*)?)$", OPEN_URL, _extract_url)
_add(r"^([\w.-]+\.(?:com|org|net|io|ai|dev|edu|gov|co|me|app))(?:\s|$)", OPEN_URL, _extract_url)
...
# ── Create file
_add(r"^(?:create|make|touch)\s+(?:a\s+)?(?:new\s+)?file\s+(?:called\s+|named\s+)?(.+)$", CREATE_FILE, _extract_filename)
_add(r"^touch\s+(.+)$", CREATE_FILE, _extract_filename)
_add(r"^(?:create|make)\s+(.+\.(?:txt|py|js|html|css|md|json|csv|xml|yaml|yml|ini|cfg|log|sh|bat|ps1))$", CREATE_FILE, _extract_filename)

# ── Create folder
_add(r"^(?:create|make)\s+(?:a\s+)?(?:new\s+)?(?:folder|directory|dir)\s+(?:called\s+|named\s+)?(.+)$", CREATE_FOLDER, _extract_folder_name)
...
# ── Delete folder
_add(r"^(?:delete|remove|rm)\s+(?:the\s+)?(?:folder|directory|dir)\s+(.+)$", DELETE_FOLDER, _extract_path)
_add(r"^rmdir\s+(.+)$", DELETE_FOLDER, _extract_path)

# ── Move / Copy / Rename
_add(r"^move\s+(.+?)\s+to\s+(.+)$", MOVE_FILE, lambda m: {"src": m.group(1).strip(), "dst": m.group(2).strip()})
_add(r"^copy\s+(.+?)\s+to\s+(.+)$", COPY_FILE, lambda m: {"src": m.group(1).strip(), "dst": m.group(2).strip()})
_add(r"^rename\s+(.+?)\s+to\s+(.+)$", RENAME_FILE, lambda m: {"path": m.group(1).strip(), "new_name": m.group(2).strip()})

# ── List directory
_add(r"^(?:list|ls|dir|show)\s+(?:files|contents|directory)\s*(?:in\s+|of\s+)?(.+)?$", LIST_DIR, _extract_path)
...
_add(r"^(?:turn|make)\s+(?:it\s+)?(?:louder|sound\s+up)(?:\s+(\d+))?$", VOLUME_UP, _extract_amount)
_add(r"^increase\s+volume(?:\s+(?:by\s+)?(\d+))?$", VOLUME_UP, _extract_amount)
_add(r"^volume\s+down(?:\s+(\d+))?$", VOLUME_DOWN, _extract_amount)
_add(r"^(?:turn|make)\s+(?:it\s+)?(?:quieter|lower|sound\s+down)(?:\s+(\d+))?$", VOLUME_DOWN, _extract_amount)
...
# ── Screenshot
_add(r"^(?:take|capture)\s+(?:a\s+)?(?:screenshot|screen\s*shot|screen\s*capture)$", TAKE_SCREENSHOT, None)

# ── System info
_add(r"^(?:show|get)\s+(?:system|pc|computer|cpu|ram|hardware)\s+info", SHOW_SYSTEM_INFO, None)

# ── Battery
_add(r"^(?:show|get|check)\s+(?:the\s+)?battery", SHOW_BATTERY, None)

# ── Kill process
_add(r"^(?:kill|close|terminate|stop|end|force quit)\s+(?:process\s+)?(.+)$", KILL_PROCESS, _extract_process)

# ── Search files
_add(r"^(?:search|find)\s+(?:files?\s+)?(?:named\s+|called\s+|for\s+)?(.+)$", SEARCH_FILES, _extract_query)
...
# ── Check website / Ping
_add(r"^(?:ping|check)\s+(?:if\s+)?(.+?\.\w{2,})(?:\s+is\s+online)?$", CHECK_WEBSITE, _extract_url)
_add(r"^(?:is)\s+(.+?\.\w{2,})\s+(?:online|up|running|working)$", CHECK_WEBSITE, _extract_url)

# ── Weather
_add(r"^weather\s+(?:in|for|at)\s+(.+)$", GET_WEATHER, _extract_city)
_add(r"^(?:what'?s?\s+the\s+)?weather(?:\s+in\s+(.+))?$", GET_WEATHER, _extract_city)
_add(r"^(?:show|get|check)\s+(?:the\s+)?weather(?:\s+(?:in|for)\s+(.+))?$", GET_WEATHER, _extract_city)
...
# ── Clipboard
_add(r"^(?:copy|set)\s+(?:to\s+)?clipboard\s*[:\-]?\s*(.+)$", SET_CLIPBOARD, _extract_clipboard_text)
...
_add(r"^download\s+(?:file\s+)?(?:from\s+)?(https?://\S+)$", DOWNLOAD_FILE, _extract_download)

# ── File manager
_add(r"^open\s+(?:file\s+)?(?:manager|explorer|finder)(?:\s+(?:at|in)\s+(.+))?$", OPEN_FILE_MANAGER, _extract_path)
...
COMPLEX_KEYWORDS = [
    "research", "analyze", "create a presentation", "write a report",
    "generate a", "build a", "develop", "summarize this",
    "explain in detail", "compare", "create a python program",
    "write code", "write a script", "make a spreadsheet",
    "create a powerpoint", "make a ppt", "draft an email",
    "send an email", "write an email", "security review",
    "vulnerability", "audit", "analyze this data", "dataset",
    "write a program", "build a website", "create a project",
    "complete my assignment", "implement", "debug", "fix the bug",
    "generate a report", "create a document", "write an essay",
]

_COMPLEX_PATTERNS = [
    re.compile(r"create\s+(?:a\s+)?(?:powerpoint|ppt|presentation|document|report|project)", re.IGNORECASE),
    re.compile(r"write\s+(?:a\s+)?(?:program|application|app|script|code|essay|report)", re.IGNORECASE),
