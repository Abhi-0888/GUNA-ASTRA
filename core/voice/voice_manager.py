...
from config.settings import (MODE_NORMAL, MODE_WORKING,
                             SPEAKER_VERIFICATION_THRESHOLD, STT_MODEL_SIZE,
                             TTS_ENGINE, TTS_VOICE_RATE,
                             USER_VOICE_EMBEDDING_PATH, VOICE_MODE_ENABLED)
...
class VoiceManager:
        ...
        try:
            ...
            if not self.tts_engine.initialize():
                ...
                self.tts_engine.speak(
                    "GUNA-ASTRA is initializing speech models. This may take a moment on the first run."
                )
    ...
    def record_command(
        self, max_seconds=10, silence_seconds=0.8, output_wav="temp_command.wav"
    ) -> str:
        """
                        ...
                        if verif_result.get("success"):
                            ...
                            if user_text:
                                ...
                                if len(filtered_text) < 3:
                                    ...
                                    print(
                                        f"[VOICE] ⚠️ Ignored junk/short transcription: '{user_text}'"
                                    )
                                    self.tts_engine.speak(
    ...
    def _run_interruptible_task(self, user_text):
        ...
        interrupter_thread = threading.Thread(
            target=self._interruption_monitor, daemon=True
        )
