...
class SpeakerVerifier:
    ...
    def initialize(self):
        ...
        try:
            ...
            if os.path.exists(self.embedding_path):
                ...
                logger.info(f"Loaded authorized voice embedding from {self.embedding_path}")
            else:
                logger.warning(f"No authorized voice embedding found at {self.embedding_path}!")
                logger.warning("Please run `scripts/enroll_voice.py` to secure the system.")
    ...
    def verify_audio(self, wav_path: str) -> dict:
        ...
        try:
            ...
            similarity = np.dot(self.authorized_embedding, new_embedding) / (
                np.linalg.norm(self.authorized_embedding) * np.linalg.norm(new_embedding)
            )
            
            is_authorized = bool(similarity >= self.threshold)
            
            logger.info(f"Voice similarity score: {similarity:.3f} (Threshold: {self.threshold})")
            if is_authorized:
    ...
    def generate_and_save_embedding(self, wav_paths: list, save_path: str = None) -> bool:
        """
        ...
        try:
            ...
            if embeddings:
                ...
                logger.info(f"✅ Successfully saved master voice profile to {save_path}")
                return True
            return False
